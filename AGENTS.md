...
- `service` 패키지의 최상위 서비스 클래스와 파일명은 모두 역할 이름 뒤에 `Service`를 붙여 통일한다. `Service`로 부르기 부자연스러운 순수 도구 타입은 예외 이름을 두지 말고 책임에 맞는 다른 패키지 배치를 먼저 검토한다.
...
- Java 코드는 작은 요청·응답 record마다 파일을 기계적으로 분리하지 않고, 동일한 리소스와 기능에 속하는 타입을 기능별 DTO 컨테이너의 `public` 중첩 타입으로 모아 관리한다.
- 클라이언트에서 서버로 들어오는 개별 타입은 `Request`, 서버에서 클라이언트로 나가는 개별 타입은 `Response`로 끝내며, WebSocket 이벤트처럼 요청·응답과 다른 역할이 명확한 타입은 그 역할을 이름에 사용한다.
- 하나의 기능별 컨테이너에 요청과 응답이 함께 있으면 `ArticleApiDto`, `ChatApiDto`처럼 방향에 치우치지 않는 `Dto` 접미사를 사용한다.
- 컨테이너의 타입이 응답 또는 요청 한 방향으로만 구성되면 `StockApiResponse`처럼 `Response` 또는 `Request`로 끝낸다.
- 같은 Controller에서 사용하는 작고 단순한 요청·응답 record는 가능한 한 하나의 기능별 DTO 컨테이너 안에 묶는다.
...
- HTML 정제는 `ArticleContentService`에서 수행하며, 정제되지 않은 사용자 입력을 화면에 렌더링하지 않는다.
